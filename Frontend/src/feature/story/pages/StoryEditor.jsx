...
const StoryEditor = ({ file, onClose, onShare }) => {
    ...
    function handleTextPointerDown(e, item) {
        ...
        // Pure stage ki jagah media element (img/video) ka size lena zaroori hai
  ...
  function handleTextPointerMove(e) {
    ...
    // 1. Move ka delta percentage nikalein
    const dxPercent = ((e.clientX - info.startX) / info.mediaWidth) * 100;
    const dyPercent = ((e.clientY - info.startY) / info.mediaHeight) * 100;

    let newX = info.origX + dxPercent;
    let newY = info.origY + dyPercent;

    // 2. Text element ki current width aur height pixels mein nikalein
    ...
    if (textEl) {
        ...
        // Element ki half-width aur half-height ko percentage mein convert karein
        ...
        // Dynamic bounds setup karein (2% padding edge se safe rakhne ke liye)
    ...
    // 3. Agar text bada hai aur bounds overlap kar rahe hain toh safety fallback lagayein
    if (minX > maxX) { minX = 5; maxX = 95; }
    if (minY > maxY) { minY = 5; maxY = 95; }

    // 4. Value ko clamp (restrict) karein taaki text boundary se bahar na jaye
        ...
        <div className="story-editor-overlay">
            <div className="story-editor-content">
                <div className="story-editor-topbar">
                    <button className="story-editor-cancel" onClick={onClose}>✕</button>
                    <button className="story-editor-add-text" onClick={handleAddTextClick}>
                        Aa Text
                    </button>
                </div>
                ...
                <div className="story-editor-stage" ref={stageRef}>
                    ...
                    {textItems.map((item) => (
                        <div
                            ...
                            onDoubleClick={() => removeTextItem(item.id)}
                        >
                            {item.value}
                        </div>
                    ))}
                ...
                {activeTextInput && (
                    <div className="story-editor-text-modal">
                        ...
                        />
                        <button onClick={confirmDraftText}>Done</button>
                    </div>
                )}
